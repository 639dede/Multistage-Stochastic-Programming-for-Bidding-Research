...
class Deterministic_Setting(pyo.ConcreteModel):
    ...
    def _BigM_setting(self):
        ...
        for t in range(self.T):   
            ...
            if self.P_da[t] >=0 and self.P_rt[t] >= 0:
                ...
                self.M_set[t][0] = (160 + self.P_da[t] + 2*self.P_rt[t])*K[t]
                self.M_set[t][1] = (80 + 2*self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][0] = (self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][1] = (80 + self.P_da[t] + self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][0] = (self.P_rt[t] + 80)*K[t]
                self.M_set_decomp[t][1][1] = (self.P_rt[t] + 80)*K[t]
                ...
                self.M_set_decomp[t][0][0] = (-self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][1] = (80 + self.P_da[t] - self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][0] = (80 - self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][1] = (80 - self.P_rt[t])*K[t]
                self.M_set_decomp[t][2][0] = max(self.M_set_decomp[t][0][0], self.M_set_decomp[t][1][0])
                self.M_set_decomp[t][2][1] = min(self.M_set_decomp[t][0][1], self.M_set_decomp[t][1][1])

            elif self.P_da[t] < 0 and self.P_rt[t] >= 0:
                
                self.M_set[t][0] = (160 + 2*self.P_rt[t])*K[t]
                self.M_set[t][1] = (80 - self.P_da[t] + 2*self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][0] = (-self.P_da[t] + self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][1] = (80 + self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][0] = (80 + self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][1] = (80 + self.P_rt[t])*K[t]
                self.M_set_decomp[t][2][0] = max(self.M_set_decomp[t][0][0], self.M_set_decomp[t][1][0])
                self.M_set_decomp[t][2][1] = min(self.M_set_decomp[t][0][1], self.M_set_decomp[t][1][1])
                ...
                self.M_set[t][0] = (160 - 2*self.P_rt[t])*K[t]
                self.M_set[t][1] = (80 - 2*self.P_da[t] - 2*self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][0] = (- self.P_da[t] - self.P_rt[t])*K[t]
                self.M_set_decomp[t][0][1] = (80 - self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][0] = (80 - self.P_rt[t])*K[t]
                self.M_set_decomp[t][1][1] = (80 - self.P_rt[t])*K[t]
    ...
    def build_model(self):    
        ...
        def rt_E_rule(model, t):
            return model.E_1[t] == E_0[t]*self.delta_E[t]
...
class Rolling_rt(pyo.ConcreteModel): # t = 0, ..., 22
    ...
    def __init__(self, t, da, b_rt, q_rt, S, E_1, T_q, delta):
        ...
        self.exp_P_rt = [P_rt[t] for t in range(self.stage, T)]
...
class fw_da(pyo.ConcreteModel): 
    ...
    def build_model(self):
        ...
        def market_clearing_1_rule(model, t):
            return model.b_da[t] - P_da[t] <= M_price*(1 - model.n_da[t])
        
        def market_clearing_2_rule(model, t):
            return P_da[t] - model.b_da[t] <= M_price*model.n_da[t]
...
class PerfectForecasting:
    ...
    def perfect_forecasting(self, scenarios):
        ...
        for k in range(self.M):
            ...
            Det = Deterministic_Setting(scenario)
                
            f_scenario = Det.get_objective_value()
...
if __name__ == "__main__":
    ...
    perfect_scenario_num = 5
    rolling_scenario_num = 5
    
    # Solve T-Stage SDDiP
    
    num_iter = 25
    
    perfect = PerfectForecasting(scenario_num = perfect_scenario_num)
