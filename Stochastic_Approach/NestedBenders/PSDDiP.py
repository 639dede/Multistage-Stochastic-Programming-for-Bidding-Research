    ...
    def build_model(self):
        ...
        def value_fcn_approx_rule(model, l):
            return model.theta <= (
                ...
                + self.psi[l][4]*model.T_q 
                + self.psi[l][5]*model.T_E
                )
    ...
    def build_model(self):
        ...
        def objective_rule(model):
            return (
                model.theta + model.f
                - (
                    sum(self.pi[0][j]*model.z_q[j] for j in range(T))
                )
            )
...
class fw_rt(pyo.ConcreteModel):
    ...
    def build_model(self):
        ...
        model.T_o = pyo.Var(domain = pyo.Reals, initialize = 0.0)
        ...
        model.m_1 = pyo.Var(domain = pyo.Reals, initialize = 0.0)
        model.m_2 = pyo.Var(domain = pyo.Reals, initialize = 0.0)
        ...
        def objective_rule(model):
            return (
                ...
                - (
                    ...
                    + self.pi[3]*model.z_T_q 
                    + self.pi[4]*model.z_T_E
                    )
            )
...
class dual_approx_sub_da(pyo.ConcreteModel): ## Subgradient method
    ...
    def add_plane(self, coeff):
        ...
        model.dual_fcn_approx.add(model.theta >= (
                lamb 
                + sum(k[0][t]*model.pi_q[t] for t in range(self.T)) 
                )
            )
...
class rolling_rt_init(pyo.ConcreteModel):
    ...
    def build_model(self):
        ...
        def minmax_rule_1_1(model, t):
            return model.m_1[t] >= self.q_da_prev[t] - model.u[t]
        
        def minmax_rule_1_2(model, t):
            return model.m_1[t] >= 0

        def minmax_rule_1_3(model, t):
            return model.m_1[t] <= self.q_da_prev[t] - model.u[t] + M_gen[t][0]*(1 - model.n_1[t])
...
class two_stage_da(pyo.ConcreteModel):
    ...
    def __init__(self, P_da_params, P_rt_exp_list):
        ...
        self.P_abs = [[0 for _ in range(T)] for _ in range(self.K)]
    ...
    def _Param_setting(self):
        ...
        for k in range(self.K):
            ...
            P_da = self.P_da_params[k]
            P_rt = self.P_rt_exp_list[k]
            
            for t in range(self.T):
                
                self.P_abs[k][t] = max(P_rt[t] - P_da[t], 0)
    ...
    def build_model(self):
        ...
        model.m_1 = pyo.Var(model.DANODE, model.TIME, domain = pyo.NonNegativeReals)
        model.m_2 = pyo.Var(model.DANODE, model.TIME, domain = pyo.NonNegativeReals)
        model.n_1 = pyo.Var(model.DANODE, model.TIME, domain = pyo.Binary)
        ...
        def rt_bidding_amount_rule(model,k ,t):
            return model.q_rt[k, t] <= E_0_partial[t] + B
        ...
        model.da_bidding_amount = pyo.Constraint(model.TIME, rule = da_bidding_amount_rule)
        model.da_overbid = pyo.Constraint(rule = da_overbid_rule)
        model.rt_bidding_amount = pyo.Constraint(model.DANODE, model.TIME, rule = rt_bidding_amount_rule)
        model.rt_overbid = pyo.Constraint(model.DANODE, rule = rt_overbid_rule)
        ...
        model.minmax_1_1 = pyo.Constraint(model.DANODE, model.TIME, rule = minmax_rule_1_1)
        model.minmax_1_2 = pyo.Constraint(model.DANODE, model.TIME, rule = minmax_rule_1_2)
        model.minmax_1_3 = pyo.Constraint(model.DANODE, model.TIME, rule = minmax_rule_1_3)
        model.minmax_1_4 = pyo.Constraint(model.DANODE, model.TIME, rule = minmax_rule_1_4)
        model.minmax_2_1 = pyo.Constraint(model.DANODE, model.TIME, rule = minmax_rule_2_1)
...
class three_stage_da(pyo.ConcreteModel):
    ...
    def __init__(self, P_da_params, scenario_paths):
        ...
        self.P_abs = [[[0 for _ in range(T)] for _ in range(self.M)] for _ in range(self.K)]
        ...
        for k in range(self.K):
            ...
            for m in range(self.M):
                ...
                for t in range(self.T):
                    ...
                    self.P_abs[k][m][t] = max(scenario_path[t][1] - P_da_list[t], 0)
    ...
    def build_model(self):
        ...
        model.m_1 = pyo.Var(model.DANODE, model.IDNODE, model.TIME, domain = pyo.NonNegativeReals)
        model.m_2 = pyo.Var(model.DANODE, model.IDNODE, model.TIME, domain = pyo.NonNegativeReals)
        model.n_1 = pyo.Var(model.DANODE, model.IDNODE, model.TIME, domain = pyo.Binary)
        ...
        def minmax_rule_1_1(model, k, m, t):
            return model.m_1[k, m, t] >= model.q_da[t] - model.u[k, m, t]
        
        def minmax_rule_1_2(model, k, m, t):
            return model.m_1[k, m, t] >= 0      
        
        def minmax_rule_1_3(model, k, m, t):
            return model.m_1[k, m, t] <= model.q_da[t] - model.u[k, m, t] + M_gen[t][0]*(1 - model.n_1[k, m, t])
        
        def minmax_rule_1_4(model, k, m, t):
            return model.m_1[k, m, t] <= M_gen[t][0]*model.n_1[k, m, t]
        
        def minmax_rule_2_1(model, k, m, t):
            return model.m_2[k, m, t] == model.m_1[k, m, t]*self.P_abs[k][m][t]
        ...
        model.rt_bidding_amount = pyo.Constraint(model.DANODE, model.IDNODE, model.TIME, rule = rt_bidding_amount_rule)
        model.rt_overbid = pyo.Constraint(model.DANODE, model.IDNODE, rule = rt_overbid_rule)
        ...
        model.minmax_1_1 = pyo.Constraint(model.DANODE, model.IDNODE, model.TIME, rule = minmax_rule_1_1)
        model.minmax_1_2 = pyo.Constraint(model.DANODE, model.IDNODE, model.TIME, rule = minmax_rule_1_2)
        model.minmax_1_3 = pyo.Constraint(model.DANODE, model.IDNODE, model.TIME, rule = minmax_rule_1_3)
        model.minmax_1_4 = pyo.Constraint(model.DANODE, model.IDNODE, model.TIME, rule = minmax_rule_1_4)
        model.minmax_2_1 = pyo.Constraint(model.DANODE, model.IDNODE, model.TIME, rule = minmax_rule_2_1)
...
def inner_product(t, pi, sol):
    ...
    return sum(pi[i]*sol[i] for i in [0, 2, 3, 4]) + sum(pi[1][j]*sol[1][j] for j in range(T - t))
...
def basic_cut_rt(t, STAGE=T):
    ...
    psi_sub = [
        ...
        0, 0, 0                     # coeffs for sol[2], sol[3], sol[4]
    ]
    return psi_sub, v0
...
def basic_pi_rt(t, STAGE=T):
    ...
    pi0 = [0, [0 for _ in range(STAGE-t)], 0, 0, 0]
    return pi0, L0
...
def process_single_subproblem_last_stage(j, prev_solution, P_da, delta, cut_mode):
    ...
    try:
        ...
        if cut_mode in ['B']:
            ...
            lag = fw_rt_last_Lagrangian([psi_sub[i] for i in range(1, 6)], P_da, delta)
            v = lag.get_objective_value()
            if v is None:
                raise ValueError("Lagrangian objective None")
...
def process_single_subproblem_inner_stage(j, t, prev_solution, psi_next, P_da, delta, cut_mode):
    ...
    try:
        ...
        if cut_mode in ['B']:
            ...
            lag = fw_rt_Lagrangian(t, [psi_sub[i] for i in range(1, 6)], psi_next, P_da, delta)
            v = lag.get_objective_value()
            if v is None:
                raise ValueError("Lagrangian objective None")
...
#### Sub processes for Lag cut generation
...
def process_lag_last_stage(j, prev_solution, P_da, delta):
    ...
    try:
        fw_lp = fw_rt_last_LP_relax(prev_solution, P_da, delta)
        pi = fw_lp.get_cut_coefficients()[1:]

        # ... your original loop ...
        gap = 1
        k_lag = [0, [0], 0, 0, 0]
        dual_subp_sub_last = dual_approx_sub(t_last, reg, pi)

        fw_lag = fw_rt_last_Lagrangian(pi, P_da, delta)
        L = fw_lag.get_objective_value()
        z = fw_lag.get_auxiliary_value()
        if L is None or z is None:
            raise ValueError("Lagrangian returned None")

        Lag_iter = 1
        obj = -1e30  # safe init so gap calc doesn't crash

        while gap >= dual_tolerance:
            if Lag_iter >= reg_num:
                dual_subp_sub_last.reg = 0

            lamb = L + inner_product(t_last, pi, z)

            for l in [0, 2, 3, 4]:
                k_lag[l] = prev_solution[l] - z[l]
            k_lag[1][0] = prev_solution[1][0] - z[1][0]

            dual_subp_sub_last.add_plane([lamb, k_lag])
            ...
            L = fw_lag.get_objective_value()
            z = fw_lag.get_auxiliary_value()
            if L is None or z is None or obj is None:
                raise ValueError("Lagrangian/dual returned None")

            pi_obj = L + inner_product(t_last, pi, prev_solution)
        ...
        return basic_pi_rt(t_last, STAGE=T)

def process_lag_inner_stage(j, t, prev_solution, psi_next, P_da, delta):
    
    try:
        fw_lp = fw_rt_LP_relax(t, prev_solution, psi_next, P_da, delta)
        pi = fw_lp.get_cut_coefficients()[1:]

        gap = 1
        k_lag = [0, [0 for _ in range(T - t)], 0, 0, 0]
        dual_subp_sub = dual_approx_sub(t, reg, pi)

        fw_lag = fw_rt_Lagrangian(t, pi, psi_next, P_da, delta)
        L = fw_lag.get_objective_value()
        z = fw_lag.get_auxiliary_value()
        if L is None or z is None:
            raise ValueError("Lagrangian returned None")
        ...
        while gap >= dual_tolerance:
            ...
            for l in [0, 2, 3, 4]:
                k_lag[l] = prev_solution[l] - z[l]
            for i in range(T - t):
                k_lag[1][i] = prev_solution[1][i] - z[1][i]

            dual_subp_sub.add_plane([lamb, k_lag])
...
class PSDDiPModel:
    ...
    def __init__(
        ...
        self.breakstage_Lag = breakstage_Lag
    ...
    def inner_product(self, t, pi, sol):
        ...
        return sum(pi[i]*sol[i] for i in [0, 2, 3, 4]) + sum(pi[1][j]*sol[1][j] for j in range(self.STAGE - t))
        ...
        for k, P_da in enumerate(self.DA_params):
            ...
            for j in range(self.N_t): 
                ...
                if self.cut_mode in BL:
                    ...
                    fw_rt_last_Lagrangian_subp = fw_rt_last_Lagrangian([psi_sub[i] for i in range(1, 6)], P_da, delta)
            ...
            for t in range(self.STAGE - 2, -1, -1): 
                ...
                v_sum = 0 
                pi_mean = [0, [0 for _ in range(self.STAGE - t)], 0, 0, 0]
                ...
                for j in range(self.N_t):
                    ...
                    if self.cut_mode in BL:
                        ...
                        fw_rt_Lagrangian_subp = fw_rt_Lagrangian(t, [psi_sub[i] for i in range(1, 6)], self.psi[k][t+1], P_da, delta)
        ...
        for k, P_da in enumerate(self.DA_params):
            ...
            for t in range(self.STAGE - 2, -1, -1): 
                ...
                for j in range(self.N_t):
                    ...
                    gap = 1
                    lamb = 0
                    k_lag = [0, [0 for _ in range(self.STAGE - t)], 0, 0, 0]
        ...
        for k, P_da in enumerate(self.DA_params):
            ...
            with mp.Pool() as pool:
                ...
                for t in range(self.STAGE - 2, -1, -1):
                    ...
                    v_sum   = 0.0
                    pi_mean = [0, [0]*(self.STAGE - t), 0, 0, 0]
    ...
    def backward_pass_Lagrangian_1(self):
        ...
        for k, P_da in enumerate(self.DA_params):
            ...
            v = psi_sub[0] - self.inner_product_da(pi, prev_da)
    ...
    def stopping_criterion(self, tol = 1e-5):
        ...
        if (
            self.running_time > self.total_time
        ):
            return True
    ...
    def run_sddip(self):
        ...
        while Timeline <= self.time_num-1:
            ...
            while True:
                ...
                if not final_pass and self.stopping_criterion():
                    ...
                    self.psi_da_save.append(copy.deepcopy(self.psi_da))
                ...
                if self.parallel_mode == 0:
                    ...
                    if self.cut_mode in ['B', 'SB']:
                        ...
                        if self.iteration <= self.breakstage_Lag:
                            self.backward_pass()
