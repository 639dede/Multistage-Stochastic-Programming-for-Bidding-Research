...
K = [1.22*E_0_partial[t] + 1.04*B for t in range(T)]
...
## stage = -1 (First bidding Real_time)     
...
class fw_rt_LP_relax(pyo.ConcreteModel): ## (Backward - Benders' Cut)
    ...
    def __init__(self, stage, T_prev, psi, delta, binary_mode):
    ...
    def build_model(self):
        ...
        def minmax_bin_rule_0_0(model):
            return model.n_4_3 == 0
        
        def minmax_bin_rule_0_1(model):
            return model.n_4 == 0
        
        def minmax_bin_rule_1_0(model):
            return model.n_4_3 == 0
        
        def minmax_bin_rule_1_1(model):
            return model.n_4 == 1
        
        def minmax_bin_rule_2_0(model):
            return model.n_4_3 == 1
        
        def minmax_bin_rule_2_1(model):
            return model.n_4 == 0
        
        def minmax_bin_rule_3_0(model):
            return model.n_4_3 == 1
        
        def minmax_bin_rule_3_1(model):
            return model.n_4 == 1
        ...
        if self.binary_mode == 0:
            model.minmax_bin_0_0 = pyo.Constraint(rule = minmax_bin_rule_0_0)
            model.minmax_bin_0_1 = pyo.Constraint(rule = minmax_bin_rule_0_1)
        
        elif self.binary_mode == 1:
            model.minmax_bin_1_0 = pyo.Constraint(rule = minmax_bin_rule_1_0)
            model.minmax_bin_1_1 = pyo.Constraint(rule = minmax_bin_rule_1_1)
            
        elif self.binary_mode == 2:
            model.minmax_bin_2_0 = pyo.Constraint(rule = minmax_bin_rule_2_0)
            model.minmax_bin_2_1 = pyo.Constraint(rule = minmax_bin_rule_2_1)
            
        elif self.binary_mode == 3:
            model.minmax_bin_3_0 = pyo.Constraint(rule = minmax_bin_rule_3_0)
            model.minmax_bin_3_1 = pyo.Constraint(rule = minmax_bin_rule_3_1)
...
class fw_rt_LP_relax_Alt(pyo.ConcreteModel): 
    ...
    def build_model(self):
        ...
        def minmax_bin_rule_0_0(model):
            return model.n_4_1 == 0
        
        def minmax_bin_rule_0_1(model):
            return model.n_4_2 == 0
        
        def minmax_bin_rule_1_0(model):
            return model.n_4_1 == 0
        
        def minmax_bin_rule_1_1(model):
            return model.n_4_2 == 1
        
        def minmax_bin_rule_2_0(model):
            return model.n_4_1 == 1
...
class fw_rt_last_LP_relax(pyo.ConcreteModel): ## stage = T (Backward)
    ...
    def __init__(self, T_prev, delta, binary_mode):
...
class SDDiPModel:
    ...
    def __init__(
        ...
        self.forward_solutions = [  ## T(-2), T(-1), ..., T(T - 2)
            [] for _ in range(self.STAGE)
        ]
        
        self.psi = [[] for _ in range(self.STAGE)] ## t = {-1 -> -2}, ..., {T - 1 -> T - 2}
        
        self._initialize_psi()

    def _initialize_psi(self):
        
        self.psi[0] = [[0 for _ in range(T)], [0 for _ in range(T)]] ## psi(-2)
    ...
    def backward_pass(self):
        ...
        for j in range(self.N_t): 
            ...
            delta = params_rt[T - 1][j]      
            
            if self.BigM_mode == 'BigM':
                fw_rt_last_LP_relax_subp = fw_rt_last_LP_relax(prev_solution, delta, 5)
            
            elif self.BigM_mode == 'Alt':
                fw_rt_last_LP_relax_subp = fw_rt_last_LP_relax_Alt(prev_solution, delta, 5)
        ...
        for t in range(self.STAGE - 2, -1, -1): 
            ...
            for j in range(self.N_t):
                ...
                if self.BigM_mode == 'BigM':
                    fw_rt_LP_relax_subp = fw_rt_LP_relax(t, prev_solution, self.psi[t+1], delta, 5)
                    
                elif self.BigM_mode == 'Alt':
                    fw_rt_LP_relax_subp = fw_rt_LP_relax_Alt(t, prev_solution, self.psi[t+1], delta, 5)         
    ...
    def backward_pass_localbin(self):
        ...
        for j in range(self.N_t): 
            ...
            for b in range(4):
                ...
                if self.BigM_mode == 'BigM':
                    fw_rt_last_LP_relax_subp = fw_rt_last_LP_relax(prev_solution, delta, b)
   
                elif self.BigM_mode == 'Alt':
                    fw_rt_last_LP_relax_subp = fw_rt_last_LP_relax_Alt(prev_solution, delta, b)
                
                psi_sub = fw_rt_last_LP_relax_subp.get_cut_coefficients()
                psi_sub_list.append(psi_sub)
                ...
                v_value = fw_rt_last_Lagrangian_subp.get_objective_value()
                v_list.append(v_value)
                
                o = 0
                
                if self.BigM_mode == 'BigM':
                    o = fw_rt_last(prev_solution, delta).get_objective_value()
              
                elif self.BigM_mode == 'Alt':
                    o = fw_rt_last_Alt(prev_solution, delta).get_objective_value()
                
                gap_list.append(v_value + sum(psi_sub[i]*prev_solution[i-1] for i in [1, 4, 5, 6, 7]) + sum(sum(psi_sub[i][t]*prev_solution[i-1][t] for t in range(1)) for i in [2, 3]) - o)
            
            min_value = min(gap_list)
            
            index_of_min = gap_list.index(min_value)
            
            psi_sub = psi_sub_list[index_of_min]
            v_value = v_list[index_of_min]
        ...
        for t in range(self.STAGE - 2, -1, -1): 
            ...
            for j in range(self.N_t):
                ...
                for b in range(4):
                    ...
                    if self.BigM_mode == 'BigM':
                        fw_rt_LP_relax_subp = fw_rt_LP_relax(t, prev_solution, self.psi[t+1], delta, b)
                 
                    elif self.BigM_mode == 'Alt':
                        fw_rt_LP_relax_subp = fw_rt_LP_relax_Alt(t, prev_solution, self.psi[t+1], delta, b)         
                    ...
                    v_value = fw_rt_Lagrangian_subp.get_objective_value()    
                    v_list.append(v_value)
                    
                    o = 0
    
                    if self.BigM_mode == 'BigM':
                        o = fw_rt(t, prev_solution, self.psi[t+1], delta).get_objective_value()
                               
                    elif self.BigM_mode == 'Alt':
                        o = fw_rt_Alt(t, prev_solution, self.psi[t+1], delta).get_objective_value()
                    
                    gap_list.append(v_value + sum(psi_sub[i]*prev_solution[i-1] for i in [1, 4, 5, 6, 7]) + sum(sum(psi_sub[i][l]*prev_solution[i-1][l] for l in range(self.STAGE - t)) for i in [2, 3]) - o)
    ...
    def stopping_criterion(self, tol = 1000):
    ...
    def run_sddip(self):
        ...
        while not self.stopping_criterion():
            ...
            if self.binary_mode == False:
                self.backward_pass()

            elif self.binary_mode == True:
                self.backward_pass_localbin()
...
if __name__ == "__main__":
    ...
    num_iter = 30
    scenario_branch_da = 2  # N_t for Day-Ahead Price
    scenario_branch_rt = 2 # N_t for Real-Time delta
    fw_scenario_num = 10  # M
    
    num_iter_list = [10, 25, 50]
    scenario_branch_list = [2, 4, 7, 10]
    fw_scenario_num_list = [2, 4, 6, 8, 10]
    BigM_mode_list = ['BigM', 'Hull', 'Alt']
    
    params_da = []
    params_rt = []
    
    """
    params_da = scenario_generator.sample_multiple_P_da(scenario_branch_da)
    
    for t in range(9, 9 + T):

        param_rt = scenario_generator.sample_multiple_delta(t, scenario_branch_rt)
        
        params_da.append()
        params_rt.append(param_rt)     
    """
    ...
    ScenarioTree1 = RecombiningScenarioTree(T+1, scenario_branch_rt, params_rt)

    
    def Comparison(BigM_mode, binary_mode):
        ...
        sddip_1 = SDDiPModel(
            ...
            backward_branch=scenario_branch_rt,
            cut_mode='B',
            BigM_mode=BigM_mode,
        ...
        plt.plot(iterations, LB_2_list, label="LB (B)", marker='o', color='tab:orange')
        ...
        print(f"SDDiP (cut='B') took {time_SDDiP_1:.2f} seconds.\n")
        print(f"SDDiP (cut='SB') took {time_SDDiP_2:.2f} seconds.\n")
        
        print(f"SDDiP optimality gap for B = {gap_SDDiP_1:.4f}")
        print(f"SDDiP optimality gap for SB = {gap_SDDiP_2:.4f}")    
    
    #for mode in BigM_mode_list:    
