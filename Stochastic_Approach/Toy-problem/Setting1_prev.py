    ...
    def build_model(self):
        ...
        def objective_rule(model):
            return (
                model.f - (
                    self.pi[0]*model.z_S 
                    + sum(self.pi[1][j]*model.z_T_b[j] for j in range(1)) 
                    + sum(self.pi[2][j]*model.z_T_Q[j] for j in range(1)) 
                    + self.pi[3]*model.z_T_q 
                    + self.pi[4]*model.z_T_b_rt 
                    + self.pi[5]*model.z_T_q_rt 
                    + self.pi[6]*model.z_T_E
                    )
            )
...
class SDDiPModel:
    ...
    def inner_product_bin(self, t, pi, sol):
        ...
        return (
            sum(
                sum(pi[i][k]*sol[i][k] for k in range(self.Round[i]+1)) for i in [0, 3, 4, 5, 6]
                ) 
            + sum(
                sum(sum(pi[i][j][k]*sol[i][j][k] for k in range(self.Round[i]+1)) for j in range(self.STAGE - t)) for i in [1, 2]
                )
            )
